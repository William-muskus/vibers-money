/**
 * Structured logger for Orchestrator. Set LOG_LEVEL=debug for verbose output.
...
function log(lvl: keyof typeof LEVELS, msg: string, data?: Record<string, unknown>): void {
  ...
  if (jsonMode) {
    console.log(JSON.stringify({ ts, level: lvl, service: 'orchestrator', message: msg, ...data }));
  } else {
    const payload = data ? ` ${JSON.stringify(data)}` : '';
    console.log(`[${ts}] [orchestrator] [${lvl.toUpperCase()}] ${msg}${payload}`);
  }
}
